...
_DEFAULT_MODELS = {
    ...
    "llama": LLAMA_DEFAULT_MODEL,
    "uf-navigator": UF_NAVIGATOR_DEFAULT_MODEL,
}

# --- App setup ---
app = FastAPI(title="Paper to Notebook", version="2.0", docs_url=None, redoc_url=None)
...
async def generate(
    ...
    api_key: str = Form(...),
    provider: str = Form("uf-navigator"),
    model: str = Form(""),
    base_url: str = Form(""),
):
    ...
    if provider not in ("openai", "llama", "uf-navigator"):
        raise HTTPException(400, "Provider must be 'openai', 'llama', or 'uf-navigator'")

    # Use default model for the selected provider if none specified
    model = model.strip() or _DEFAULT_MODELS.get(provider, OPENAI_DEFAULT_MODEL)
